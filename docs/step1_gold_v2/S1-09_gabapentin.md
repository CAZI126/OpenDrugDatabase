...
The V1 gold cited TABLE 1 but listed only two of its bands. The question asks for the table, so every row and every footnote belongs in the gold. The V1 record and the V1 verdict are unchanged; this addition applies to the next run only.

### TABLE 1. NEURONTIN Dosage Based on Renal Function — every row

| Renal function, creatinine clearance (mL/min) | Total daily dose range (mg/day) | Dose regimen (mg) |
| --- | --- | --- |
| CrCl ≥ 60 | 900 to 3600 | 300 TID, 400 TID, 600 TID, 800 TID, 1200 TID |
| CrCl >30 to 59 | 400 to 1400 | 200 BID, 300 BID, 400 BID, 500 BID, 700 BID |
| CrCl >15 to 29 | 200 to 700 | 200 QD, 300 QD, 400 QD, 500 QD, 700 QD |
| CrCl 15 | 100 to 300 | 100 QD, 125 QD, 150 QD, 200 QD, 300 QD |
| Hemodialysis | post-hemodialysis supplemental dose | 125, 150, 200, 250, 350 |

### Footnotes carried with the table
...
- **CrCl ≥ 60: total daily dose 900 to 3600 mg/day; doses 300 TID, 400 TID, 600 TID, 800 TID, 1200 TID.**
  - supported by: the TABLE 1 row `≥ 60 900 to 3600 300 TID, 400 TID, 600 TID, 800 TID, 1200 TID`
- **CrCl >30 to 59: total daily dose 400 to 1400 mg/day; doses 200 BID, 300 BID, 400 BID, 500 BID, 700 BID.**
  - supported by: the TABLE 1 row `>30 to 59 400 to 1400 200 BID, 300 BID, 400 BID, 500 BID, 700 BID`
- **CrCl >15 to 29: total daily dose 200 to 700 mg/day; doses 200 QD, 300 QD, 400 QD, 500 QD, 700 QD.**
  - supported by: the TABLE 1 row `>15 to 29 200 to 700 200 QD, 300 QD, 400 QD, 500 QD, 700 QD`
- **CrCl 15: total daily dose 100 to 300 mg/day; doses 100 QD, 125 QD, 150 QD, 200 QD, 300 QD.**
  - supported by: the TABLE 1 row `15 100 to 300 100 QD, 125 QD, 150 QD, 200 QD, 300 QD`
- **Hemodialysis: supplemental dose after each 4 hours of hemodialysis, 125, 150, 200, 250, 350 mg.**
  - supported by: `Hemodialysis 125 150 200 250 350` with the footnote `a supplemental post-hemodialysis dose administered after each 4 hours of hemodialysis`
- **Below CrCl 15, reduce the daily dose in proportion to creatinine clearance.**
  - supported by: `For patients with creatinine clearance <15 mL/min, reduce daily dose in proportion to creatinine clearance (e.g., patients with a creatinine clearance of 7.5 mL/min should receive one-half the daily dose that patients with a creatinine clearance of 15 mL/min receive).`
- **Patients on hemodialysis take maintenance doses based on estimated creatinine clearance from the upper part of the table.**
  - supported by: `Patients on hemodialysis should receive maintenance doses based on estimates of creatinine clearance as indicated in the upper portion of the table`

No band, total daily dose, or individual dose from the table is omitted.
